...
router.post('/dashboard', async (req, res) => {
  ...
  if (sess == null) {
    ...
    webpush.sendNotification(sess.subscription).catch(error => {
    ...
    for (var abmelden in req.body) {
      if (abmelden.split('_')[0] == "abmelden") {
        ...
        await sequelize
          .query(' call delete_students_has_pruefungs(:stud_id, :pruef_id)', {
            replacements: {
        ...
        if (modul_loeschen) {
          ...
          await sequelize
            .query(' call delete_students_has_moduls(:stud_id, :modul_id)', {
              replacements: {
                stud_id: sess.nutzer.id,
                modul_id: modul.id,
              }
